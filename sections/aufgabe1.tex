...
\begin{itemize}
    \item Regeln sind für mich ein Tool, um die Grenzen in der WG und für mich abzustecken (wie weit ist mein Handlungs-Spielraum)
    \item ich bin die einzige die Regel für sinnvoll hält
    \item Ich versuche das Werkzeug zu Verkaufen
    \item Ich bin \textbf{emotional engagiert}
\end{itemize}
...
\begin{mybox}
\begin{itemize}
    \item Robert sieht, laut eigenen Angaben keine Notwendigkeit zum Handeln, da es ja super funktioniert
    \item Tilo appelliert stetig an die eigene Verantwortung
\end{itemize}
\end{mybox}
\begin{mybox}
    \begin{center}
        Aufgrund der Unterschiedlichen Grundeinstellungen der beiden, sehe ich da ein immer anhaltendes Problem mit Steigerung-Potenzial!\\
        \vspace{1em}
        Daher habe ich mir zur Aufgabe gemacht die Situation vorher anzugehen und sie zu entschärfen.\\
        \vspace{1em}
        Notiz für mich, wenn das Werkzeug von keinem akzeptiert wird: \\ 
        \noindent
        {\color{red} Nicht persönlich nehmen! }
    \end{center}
\end{mybox}